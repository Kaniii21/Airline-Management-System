...
const char l_destination[5][20]={"KOLKATA","DELHI","CHENNAI","GOA","MUMBAI"};	//local Destination
const char i_destination[11][20]={"LONDON","BERLIN","DUBAI","BANGKOK","BEIJING","PARIS","CANBERRA","TOKYO","WASHINGTON","JAPAN","SINGAPORE" }; //International Destination
...
class booking:public passenger
{
	...
	bool ldest()
	{
		...
		cout<<endl<<"THE LIST OF LOCAL DESTINATIONS WE ARE FLYING: \n";
		for(i=0;i<5;i++)
		{
			for(j=0;j<20;j++)
			{
		...
		cin>>origin;
		cout<<endl<<"Enter your destination(. BLOCK letters): ";
		cin>>destination;
	...
	{
		passport = pass;
		cout<<endl<<"THE LIST OF INTERNATIONAL DESTINATIONS WE ARE FLYING: \n";
		for(i=0;i<11;i++)
		{
			for(j=0;j<20;j++)
			{
		...
		again:
		cout<<endl<<"Enter your origin(BLOCK letters): ";
		cin>>origin;
		cout<<endl<<"Enter your destination(BLOCK letters): ";
		cin>>destination;
		if(strcmp(origin,destination)==0)
		...
		void seats()
		{
			...
			{
				cout<<endl<<"Sorry there are no seats available :( \n Hope you have a nice day!";
				exit(0);
			}
		}
				...
				if(y==v)
				{
					...
					switch(option)
					{
						case 49:
						{
							...
							cout<<endl<<"Enter the new date of departure(DD/MM/YYYY): ";
							cin>>c2;
...
class about
{
	public:
		void Aboutus()
		{
			cout<<"The height of skies was first touched by our airline in 1977. Since then,\n KB FLY is the world's leading airline with 4 biggest airplanes.\nWe are at the customers high service in cuisine, comfort and care.\nSo make a plan and fly high with High Fly."<<endl<<endl;
...
int main()
{
	...
	back2:
	cout<<endl<<"\t\tEnter 1 for Booking"
	<<endl<<"\t\tEnter 2 for Routes"
	<<endl<<"\t\tEnter 3 for Complaint"
	<<endl<<"\t\tEnter 4 for About"
	<<endl<<"\t\tEnter 5 for Manage"
	<<endl<<"\t\tEnter 0 for Exit\n";
	choice = getch();
	system("cls");
	switch(choice)
	{
		case 49:
			{
				...
				getline(cin,l_name);
				cout<<"Enter Your Address(. Commas only): ";
				getline(cin,add);
